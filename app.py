    ...
    if st.session_state.teacher_mode:
        ...
        if st.session_state.problems:
            for i, problem in enumerate(st.session_state.problems):
                ...
                with col1:
                    if st.button(f"P{i+1}", use_container_width=True,
...
ASSESS: Identify language proficiency (beginner/intermediate/advanced) and math understanding.

SCAFFOLD:
- Praise effort and correct thinking
- Ask questions instead of giving answers
- Use simple language for ESOL students
- Provide sentence starters: "The problem asks me to...", "I need to find...", "The operation is..."

GUIDE through steps:
1. Understanding: What is the problem asking?
2. Identifying: What information do we have? What are we solving for?
3. Planning: What operation will we use?
4. Solving: Work through the math
5. Checking: Does our answer make sense?

ADAPT your language based on proficiency level.
...
Help the student based on where they are in their thinking."""
    ...
    if st.session_state.problems:
        st.success(f"✅ {len(st.session_state.problems)} problem(s) ready")
    else:
        st.info("Add word problems using the sidebar")
```

## **File 2: requirements.txt** (stay the same)
```
streamlit
anthropic
openai